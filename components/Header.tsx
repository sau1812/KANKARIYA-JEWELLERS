...
const Header = () => {
  ...
  // Nayi states Scroll aur Search ke liye
  ...
  useEffect(() => {
    const fetchCoupon = async () => {
      try {
        const data = await client.fetch(`*[_type == "coupon" && isActive == true][0]{code, discountPercentage}`);
        setCoupon(data);
      } catch (error) {
  ...
  return (
    <header className='sticky top-0 w-full z-50 bg-[#FCFBF8]/95 backdrop-blur-2xl border-b border-gray-200 transition-all duration-500'>
  ...
  <Container>
    {coupon ? (
      ...
      <Container className={`relative flex items-center justify-between transition-all duration-500 ${isScrolled ? 'py-3 md:py-3' : 'py-4 md:py-6'}`}>
        ...
        {/* --- 4. RIGHT SECTION (Icons: Search, Wishlist, User, Bag) --- */}
        <div className="flex items-center gap-x-5 md:gap-x-7 relative z-10 text-stone-800">
            ...
            {/* WISHLIST ICON (Functional Component Restored) */}
            ...
            {/* SHOPPING BAG (Functional Component Restored) */}
            <div className="flex items-center cursor-pointer hover:text-kankariya-gold transition-colors duration-500 group">
                {/* Aapka CartIcon yahan rakha hai */}
                <CartIcon />
                
                {/* Note: Agar aap chahte hain ki 'Bag' text likha aaye jaise pehle tha, 
                    toh use apne 'CartIcon' file ke andar hi add karna padega taaki dono pe click chale. */}
            </div>
