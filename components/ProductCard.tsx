...
function ProductCard({ item }: ProductProps) {
  ...
  const discountPercentage = isSale && item.originalPrice
        ...
        <div className="relative w-full aspect-[4/5] bg-stone-100 overflow-hidden">
            ...
            {!isOutOfStock && (
                <div className="absolute top-2 left-2 flex flex-col gap-1 z-10">
                    ...
                    {isSale && (
                        <span className="flex items-center gap-1 bg-white/90 text-rose-600 text-[9px] font-bold px-1.5 py-0.5 rounded shadow-sm w-fit backdrop-blur-[2px]">
                            <Sparkles size={10} /> -{discountPercentage}%
                        </span>
                    )}
                </div>
            )}
